...
var attendeeslist = new Array( "kthompsp" ,  "paolaricaurte" ,  "davidsancar" ,  "jvitak" ,  "katieshilton" ,
...
function tagcloud(div, cl) {	 
   ...
   var color = d3.scale.linear().domain([mintweet,maxtweet]).range(['#B8860B',"yellowgreen"]);   
   ...
   h = h/12;
    ...
    curuserdata = userdata.filter(function(val){
   	if(showAttendees=='all'){ 
   		...
   		return ($.inArray(val.screen_name,attendeeslist)==-1);
   	}
...
function aggregate(data0){
	
var bin=15*60*1000;//15 minute	
var start ;
var end;
var length = (end-start)/duration;
var binValue = [length];// of size Length
...
data0.forEach(function(d) {
			// console.log("loop:: "+d.tweet_time);
     		timebin = format.parse(d.created_at);
     		timebin = +timebin;// in mili second //
     		var position = (timebin-start)/duration;
     		// console.log("again "+d.tweet_time);
     		tweets_per_bin[position] = tweets_per_bin[position]+1;
		});	
}
...
var data0 = usertweets;         

data0.forEach(function(d) {
			// console.log("loop:: "+d.tweet_time);
     		d.created_at = format.parse(d.created_at );
     		// console.log("again "+d.tweet_time);
     		d.total = +d.total;
		});
...
.attr("shape-rendering","crispEdges")
.attr("transform","translate( -10,"+ 10 +"), rotate(-)");
...
if( duration.indexOf("session")!=-1){
	...
	var session = duration.substring("session".length);
		...
		if(e-d <= 2*24*60*60*1000){
			parse2 =  d3.time.format("%d-%b %I%p"); 
			// xAxis = d3.svg.axis().scale(x).tickFormat(parse2).tickSize(4).ticks(d3.time.days, 1);//.tickSubdivide(true);
			xAxis.tickFormat(parse2).ticks(d3.time.hours, 2);
...
// //    
   ...
   // .style("background-color","white")
       // .selectAll("div")
   					// .data( function(d){ return getTags(d);} )   					  										
   					// .enter().append("div")            		
   					// .style("line-height","0.85em")	       
  					// .attr("class","tagcloudlink")
   					// .style("display","block")   					
    ...
    $.ajax({
       ...
       if(userFilter !=''){
		...
		if(showAttendees == "only"){
    		...
    		usertweets = usertweets.filter(function(val) {  
    		// console.log(val.screen_name);   	    		 
				return ( ($.inArray(val.screen_name,attendeeslist)>=0)); // diff in milli seconds
		});	
    		...
    		usertweets = usertweets.filter(function(val) {  
    		// console.log(val.screen_name);   	    		 
				return ( ($.inArray(val.screen_name,attendeeslist)==-1)); // diff in milli seconds
		});	
