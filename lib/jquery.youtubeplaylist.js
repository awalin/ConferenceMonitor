...
jQuery.fn.ytplaylist = function(options) {
  ...
  return this.each(function() {
		...
		function playOld(id) {
		   ...
		   // ATR: added two lines to support play on click
		   if(options.autoPlay && options.playOnLoad) autoPlay = "&autoplay=1";
		   options.playOnLoad = true;

		   html += '<object height="'+options.playerHeight+'" width="'+options.playerWidth+'">';
		   // ATR - hack added autohide
		   //html += '<param name="movie" value="http://www.youtube.com/v/'+id+autoPlay+showRelated+fullScreen+'"> </param>';
		   html += '<param name="movie" value="http://www.youtube.com/v/'+id+'?version=3&autohide=1&showinfo=0&controls=0'+autoPlay+showRelated+fullScreen+'"></param>';
		   html += '<param name="allowScriptAccess" value="always"></param>';
		   html += '<param name="wmode" value="transparent"> </param>';
		   if(options.allowFullScreen) { 
		   		html += '<param name="allowfullscreen" value="true"> </param>'; 
		   }
		   // ATR - hack: added allowscriptaccess line too
		   //html += '<embed src="http://www.youtube.com/v/'+id+autoPlay+showRelated+fullScreen+'"';
		   html += '<embed src="http://www.youtube.com/v/'+id+'?version=3&autohide=1&showinfo=0&controls=0'+autoPlay+showRelated+fullScreen+'"';
		   if(options.allowFullScreen) { 
		...
		// ATR: changed to use dt instead of li
		//$el.children('li').each(function() {
		$el.children('dt').each(function() {
		...
		$el.children("dt").children("a.yt-vid").click(function() {
			
			if(options.showInline) {
				// ATR: changed to use dt instead of li
				//$("li.currentvideo").removeClass("currentvideo");
				//$(this).parent("li").addClass("currentvideo").html(playOld($(this).data("yt-id")));
				$("dt.currentvideo").removeClass("currentvideo");
				$(this).parent("dt").addClass("currentvideo").html(playOld($(this).data("yt-id")));
			}
			else {
				...
				$(this).parent().parent("dl").find("dt.currentvideo").removeClass("currentvideo");
				$(this).parent("dt").addClass("currentvideo");
			}	
			return false;
		});
		...
		$el.find("a.img-link").click(function() {
		    ...
		    if(options.showInline) {
		        ...
		        $("dt.currentvideo").removeClass("currentvideo");
		        $(this).parent("dt").addClass("currentvideo").html($img);
	        }
	        else {
				...
				//$(this).parent("li").addClass("currentvideo");
				$(this).closest("dl").find("dt.currentvideo").removeClass("currentvideo");
				$(this).parent("dt").addClass("currentvideo");
		...
		if(options.addThumbs) {
			...
			$el.children().each(function(i){
				...
				// ATR: added width and height parameters so thumb fits player dimensions
    				var thumbHtml = "<img src='"+thumbUrl+"' width='"+options.playerWidth+"' height='"+options.playHeight+"' alt='"+replacedText+"' />";
		...
		var firstVid = $el.children("dt:first-child").addClass("currentvideo").children("a").click();
